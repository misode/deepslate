...
<SplinePlot name="Peaks from continentalness [-0.3, 0.3]" width={WIDTH} height={HEIGHT} range={[-0.3, 0.3]}
	spline={x => TerrainShaper.peaks(TerrainShaper.point(x, -0.8, 0.6))}
/>

<SplinePlot name="Peaks from ridges [0, 1]" width={WIDTH} height={HEIGHT} range={[0, 1]}
	spline={x => TerrainShaper.peaks({ continents: 0.11, erosion: -0.45, weirdness: 0, ridges: x })}
/>
