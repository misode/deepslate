...
const noiseSampler = new NoiseSampler(4, 4, 32, biomeSource, NoiseSettings.fromJson(null),netherOctaves, BigInt(5392))
const climateSampler = (x, y, z) => noiseSampler.getClimate(x, y, z)
...
<Plot name="Nether" width={160} scale={2}
	initializer={() => {
		...
		const sampler = new NoiseSampler(4, 4, 32, source, NoiseSettings.fromJson(null), netherOctaves, BigInt(5392))
		const climate = (x, y, z) => sampler.getClimate(x, y, z)
...
<Plot name="Overworld" width={160} scale={2}
	...
	initializer={data => {
		...
		const sampler = new NoiseSampler(4, 4, 32, source, NoiseSettings.fromJson(null), overworldOctaves, BigInt(5392))
		const climate = (x, y, z) => sampler.getClimate(x, y, z)
		return { source, climate }
	}}
	sampler={({ source, climate }) => (x, y) => source.getBiome(x, 64, y, climate)}
