...
export abstract class DensityFunction
  implements MinMaxNumberFunction<DensityFunction.Context>
{
...
export namespace DensityFunction {
  ...
  abstract class Transformer extends DensityFunction {
    constructor(public readonly input: DensityFunction) {
      super();
    }
  ...
  const NoiseParser = Holder.parser(
    WorldgenRegistries.NOISE,
    NoiseParameters.fromJson
  );

  export function fromJson(
    obj: unknown,
    inputParser: (obj: unknown) => DensityFunction = fromJson
  ): DensityFunction {
  ...
  export class HolderHolder extends DensityFunction {
    constructor(public readonly holder: Holder<DensityFunction>) {
      super();
    }
    public compute(context: Context): number {
  ...
  abstract class Wrapper extends DensityFunction {
    constructor(protected readonly wrapped: DensityFunction) {
      super();
    }
    public minValue() {
  ...
  export class FlatCache extends Wrapper {
    ...
    public compute(context: Context): number {
      ...
      if (this.lastQuartX !== quartX || this.lastQuartZ !== quartZ) {
        this.lastValue = this.wrapped.compute(
          DensityFunction.context(quartX << 2, 0, quartZ << 2)
        );
  ...
  export class Interpolated extends Wrapper {
    ...
    public compute({
      ...
      const noise111 = () =>
        this.computeCorner(firstX + w, firstY + h, firstZ + w);
      return lazyLerp3(
        x,
        y,
        z,
  ...
  export class Noise extends DensityFunction {
    ...
    public compute(context: Context) {
      return (
        this.noise?.sample(
          context.x * this.xzScale,
          context.y * this.yScale,
          context.z * this.xzScale
        ) ?? 0
      );
    }
    public maxValue() {
  ...
  export class EndIslands extends DensityFunction {
    ...
    private getHeightValue(x: number, z: number) {
      ...
      for (let i = -12; i <= 12; i += 1) {
        for (let j = -12; j <= 12; j += 1) {
          ...
          if (
            x2 * x2 + z2 * z2 <= 4096 ||
            this.islandNoise.sample2D(x2, z2) >= -0.9
          ) {
            continue;
          }
    ...
    public compute({ x, y, z }: DensityFunction.Context) {
      return (
        (this.getHeightValue(Math.floor(x / 8), Math.floor(z / 8)) - 8) / 128
      );
    }
    public minValue() {
      return -0.84375;
  ...
  export class WeirdScaledSampler extends Transformer {
    private static readonly ValueMapper: Record<
      (typeof RarityValueMapper)[number],
      (value: number) => number
    > = {
  ...
  export class RangeChoice extends DensityFunction {
    ...
    public minValue() {
      return Math.min(
        this.whenInRange.minValue(),
        this.whenOutOfRange.minValue()
      );
    }
    public maxValue() {
      return Math.max(
        this.whenInRange.maxValue(),
        this.whenOutOfRange.maxValue()
      );
    }
  }
  ...
  export class Clamp extends Transformer {
    ...
    public mapAll(visitor: Visitor): DensityFunction {
      return visitor.map(
        new Clamp(this.input.mapAll(visitor), this.min, this.max)
      );
    }
    public minValue() {
      return this.min;
  ...
  export class Mapped extends Transformer {
    private static readonly MappedTypes: Record<
      (typeof MappedType)[number],
  ...
  export class Ap2 extends DensityFunction {
    ...
    public compute(context: Context) {
      ...
      switch (this.type) {
        case "add":
          return a + this.argument2.compute(context);
        case "mul":
          return a === 0 ? 0 : a * this.argument2.compute(context);
        case "min":
          return a < this.argument2.minValue()
            ? a
            : Math.min(a, this.argument2.compute(context));
        case "max":
          return a > this.argument2.maxValue()
            ? a
            : Math.max(a, this.argument2.compute(context));
        case "subtract":
          return a - this.argument2.compute(context);
      }
    }
    ...
    public withMinMax() {
      ...
      if (
        ...
        console.warn(
          `Creating a ${this.type} function between two non-overlapping inputs`
        );
      }
      let min, max;
      switch (this.type) {
  ...
  export class Spline extends DensityFunction {
    constructor(public readonly spline: CubicSpline<Context>) {
      super();
    }
    public compute(context: Context) {
  ...
  abstract class ErrorableOperator extends DensityFunction {
    ...
    public compute(context: Context): number {
      return clamp(
        this.computeWrapped(context, this.input.compute(context)) ??
          this.error_output == null
          ? 0
          : this.error_output.compute(context),
        this.min_output ?? -Infinity,
        this.max_output ?? Infinity
      );
    }
  }
