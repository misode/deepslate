...
export namespace DensityFunction {
	...
	export function fromJson(obj: unknown): DensityFunction {
		return create(obj, fromJson)
	}

	export function create(obj: unknown, input_parser: (obj: unknown) => DensityFunction): DensityFunction {
		...
		switch (type) {
			...
			case 'flat_cache': return new FlatCache(input_parser(root.argument))
			case 'interpolated': return new Interpolated(input_parser(root.argument))
			case 'cache_2d': return new Cache2D(input_parser(root.argument))
			case 'cache_once': return new CacheOnce(input_parser(root.argument))
			case 'cache_all_in_cell': return new CacheAllInCell(input_parser(root.argument))
			...
			case 'weird_scaled_sampler': return new WeirdScaledSampler(
				input_parser(root.input),
				Json.readEnum(root.rarity_value_mapper, RarityValueMapper),
				NoiseParser(root.noise),
			)
			case 'shifted_noise': return new ShiftedNoise(
				input_parser(root.shift_x),
				input_parser(root.shift_y),
				input_parser(root.shift_z),
				Json.readNumber(root.xz_scale) ?? 1,
				Json.readNumber(root.y_scale) ?? 1,
				NoiseParser(root.noise),
			)
			case 'range_choice': return new RangeChoice(
				...
				input_parser(root.when_in_range),
				input_parser(root.when_out_of_range),
			)
			...
			case 'blend_density': return new BlendDensity(input_parser(root.argument))
			...
			case 'squeeze':
				return new Mapped(type, input_parser(root.argument))
			case 'slide': return new Slide(input_parser(root.argument))
			case 'add':
			case 'mul':
			case 'min':
			case 'max': return new Ap2(
				Json.readEnum(type, Ap2Type),
				input_parser(root.argument1),
				input_parser(root.argument2),
			)
			case 'spline': return new Spline(
				CubicSpline.fromJson(root.spline, input_parser),
			...
			case 'terrain_shaper_spline': return new TerrainShaperSpline(
				input_parser(root.continentalness),
				input_parser(root.erosion),
				input_parser(root.weirdness),
