...
describe('DensityFunction', () => {
	...
	it('Abs', () => {
		const fn1 = wrap(new DF.Unary('abs', new DF.Constant(2)))
		expect(fn1.compute(ContextA)).toEqual(2)
		const fn2 = wrap(new DF.Unary('abs', new DF.Constant(-3)))
		expect(fn2.compute(ContextA)).toEqual(3)
	})

	it('Square', () => {
		const fn1 = wrap(new DF.Unary('square', new DF.Constant(2)))
		expect(fn1.compute(ContextA)).toEqual(4)
		const fn2 = wrap(new DF.Unary('square', new DF.Constant(-3)))
		expect(fn2.compute(ContextA)).toEqual(9)
	})

	it('Cube', () => {
		const fn1 = wrap(new DF.Unary('cube', new DF.Constant(2)))
		expect(fn1.compute(ContextA)).toEqual(8)
		const fn2 = wrap(new DF.Unary('cube', new DF.Constant(-3)))
		expect(fn2.compute(ContextA)).toEqual(-27)
	})

	it('Half negative', () => {
		const fn1 = wrap(new DF.Unary('half_negative', new DF.Constant(2)))
		expect(fn1.compute(ContextA)).toEqual(2)
		const fn2 = wrap(new DF.Unary('half_negative', new DF.Constant(-3)))
		expect(fn2.compute(ContextA)).toEqual(-1.5)
	})

	it('Quarter negative', () => {
		const fn1 = wrap(new DF.Unary('quarter_negative', new DF.Constant(2)))
		expect(fn1.compute(ContextA)).toEqual(2)
		const fn2 = wrap(new DF.Unary('quarter_negative', new DF.Constant(-3)))
		expect(fn2.compute(ContextA)).toEqual(-0.75)
	})

	it('Squeeze', () => {
		const fn1 = wrap(new DF.Unary('squeeze', new DF.Constant(2)))
		expect(fn1.compute(ContextA)).toEqual(0.4583333333333333)
		const fn2 = wrap(new DF.Unary('squeeze', new DF.Constant(0.5)))
		expect(fn2.compute(ContextA)).toEqual(0.24479166666666666)
		const fn3 = wrap(new DF.Unary('squeeze', new DF.Constant(-0.7)))
