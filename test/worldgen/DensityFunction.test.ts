...
describe('DensityFunction', () => {
	const ContextA = DF.context(1, 2, 3, 4, 8)
	const ContextB = DF.context(2, 3, 4, 4, 8)
	const ContextC = DF.context(12, -30, 1, 4, 8)
	...
	it('Clamp', () => {
		...
		expect(fn.compute(DF.context(0, 0, 0, 4, 8))).equal(2)
		expect(fn.compute(DF.context(0, 3, 0, 4, 8))).equal(3)
		expect(fn.compute(DF.context(0, 7, 0, 4, 8))).equal(5)
	})
	...
	it('Slide', () => {
		...
		expect(fn.compute(DF.context(0, 40, 0, 4, 8))).equal(0.5)
		expect(fn.compute(DF.context(0, -64, 0, 4, 8))).equal(10)
		expect(fn.compute(DF.context(0, 319, 0, 4, 8))).equal(-5)
		expect(fn.compute(DF.context(0, -55, 0, 4, 8))).equal(5.25)
		expect(fn.compute(DF.context(0, 305, 0, 4, 8))).equal(-2.25)
		expect(fn.compute(DF.context(0, 290, 0, 4, 8))).equal(0.5)
	})
	...
	it('Spline', () => {
		...
		expect(fn2.compute(DF.context(0, 0, 0, 4, 8))).equal(1)
		expect(fn2.compute(DF.context(0, 3.2, 0, 4, 8))).equal(0.4363904)
		expect(fn2.compute(DF.context(0, 5, 0, 4, 8))).equal(0.2)
		expect(fn2.compute(DF.context(0, 11, 0, 4, 8))).equal(0.376)
		expect(fn2.compute(DF.context(0, 20, 0, 4, 8))).equal(0.7)
		expect(fn2.compute(DF.context(0, 25, 0, 4, 8))).equal(0.7)
	})

	it('YClampedGradient', () => {
		const fn1 = wrap(new DF.YClampedGradient(0, 128, 0, 128))
		expect(fn1.compute(DF.context(0, -5, 0, 4, 8))).equal(0)
		expect(fn1.compute(DF.context(0, 0, 0, 4, 8))).equal(0)
		expect(fn1.compute(DF.context(0, 4, 0, 4, 8))).equal(4)
		expect(fn1.compute(DF.context(0, 127, 0, 4, 8))).equal(127)
		expect(fn1.compute(DF.context(0, 128, 0, 4, 8))).equal(128)
		expect(fn1.compute(DF.context(0, 129, 0, 4, 8))).equal(128)
		const fn2 = wrap(new DF.YClampedGradient(0, 128, -16, 0))
		expect(fn2.compute(DF.context(0, -200, 0, 4, 8))).equal(-16)
		expect(fn2.compute(DF.context(0, 0, 0, 4, 8))).equal(-16)
		expect(fn2.compute(DF.context(0, 5, 0, 4, 8))).equal(-15.375)
		expect(fn2.compute(DF.context(0, 64, 0, 4, 8))).equal(-8)
		expect(fn2.compute(DF.context(0, 128, 0, 4, 8))).equal(0)
		expect(fn2.compute(DF.context(0, 129, 0, 4, 8))).equal(0)
	})
})
