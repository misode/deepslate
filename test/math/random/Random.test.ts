...
describe('Random', () => {
	it('nextInt', () => {
		const random = new Random(BigInt(123))
	...
	it('nextInt (max)', () => {
		...
		expect(random.nextInt(256)).to.equal(185)
		expect(random.nextInt(255)).to.equal(200)
		expect(random.nextInt(254)).to.equal(74)
	})
	...
	it ('nextFloat', () => {
		...
		actual.forEach((a, i) => {
			expect(a).to.be.approximately(expected[i], 1e-7)
		})
	})
	...
	it ('nextDouble', () => {
		...
		actual.forEach((a, i) => {
			expect(a).to.be.approximately(expected[i], 1e-8)
		})
	})
})
